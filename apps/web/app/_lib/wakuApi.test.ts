...
describe("getWaku", () => {
  test("一覧を取る", async () => {
    const fetchImpl = vi.fn<typeof fetch>(async (input) => {
      expect(String(input)).toBe("/waku");
      return Response.json([item]);
    });
    await expect(getWaku({ fetch: fetchImpl })).resolves.toEqual({ ok: true, value: [item] });
  });

  test("配列以外は受け取らない", async () => {
    const fetchImpl = vi.fn<typeof fetch>(async () => Response.json({}));
    await expect(getWaku({ fetch: fetchImpl })).resolves.toEqual({
      ok: false,
      error: "帳簿が読めません",
    });
  });
...
describe("postWaku / putWaku", () => {
  test("足すと改める", async () => {
    ...
    const fetchPut = vi.fn<typeof fetch>(async (input, init) => {
      expect(String(input)).toBe("/waku/w1");
      expect(init?.method).toBe("PUT");
      return Response.json({ id: "w1", name: "日用品" });
    });
    await expect(postWaku("食費", { fetch: fetchPost })).resolves.toEqual({
      ok: true,
      value: item,
    });
    await expect(putWaku("w1", "日用品", { fetch: fetchPut })).resolves.toEqual({
      ok: true,
