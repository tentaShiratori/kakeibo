...
3. 置き場所は `architecture.mdc`。`apps/web` と `apps/app` は既存の隣。`api` は `internal/` の構成に、今要るフォルダだけ足す（空の mysql / s3 / master_data は作らない）。書く処理は usecase、読む処理は query、HTTP は controller
