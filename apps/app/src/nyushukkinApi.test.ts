...
describe("getNyushukkin", () => {
  test("暦月の一覧を取る", async () => {
    const fetchImpl = vi.fn<typeof fetch>(async (input) => {
      expect(String(input)).toBe(`${origin}/nyushukkin?month=2026-09`);
      return Response.json([item]);
    });

    await expect(getNyushukkin("2026-09", { fetch: fetchImpl, apiUrl: origin })).resolves.toEqual({
      ok: true,
      value: [item],
    });
  });
...
describe("postNyushukkin", () => {
  test("記録を送り、api が付けた id を受け取る", async () => {
    const fetchImpl = vi.fn<typeof fetch>(async (input, init) => {
      expect(String(input)).toBe(`${origin}/nyushukkin`);
...
describe("putNyushukkin / deleteNyushukkin", () => {
  test("直すと消すを id つきで送る", async () => {
    const fetchPut = vi.fn<typeof fetch>(async (input, init) => {
      expect(String(input)).toBe(`${origin}/nyushukkin/a`);
    ...
    await expect(
      putNyushukkin(
        ...
        { fetch: fetchPut, apiUrl: origin },
      ),
    ).resolves.toEqual({ ok: true, value: { ...item, amount: 3000 } });
    await expect(deleteNyushukkin("a", { fetch: fetchDelete, apiUrl: origin })).resolves.toEqual({
      ok: true,
      value: item,
    });
  });
...
describe("失敗", () => {
  test("api の error をそのまま出す", async () => {
    ...
    await expect(
      deleteNyushukkin("missing", { fetch: fetchImpl, apiUrl: origin }),
    ).resolves.toEqual({
      ok: false,
      error: "その入出金はありません",
    });
