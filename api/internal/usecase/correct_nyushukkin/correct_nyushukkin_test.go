...
func (s *stubNyushukkin) Record(item nyushukkin.Nyushukkin) error { return nil }
func (s *stubNyushukkin) Correct(item nyushukkin.Nyushukkin) error {
	s.items = []nyushukkin.Nyushukkin{item}
	return nil
}
...
func TestCorrectNyushukkinAttachAndDetach(t *testing.T) {
	n := &stubNyushukkin{}
	w := &stubWaku{items: []waku.Waku{{ID: "w1", Name: "食費"}}}
	got, err := CorrectNyushukkin(testApp(n, w), "n1", nyushukkin.Input{
...
func TestCorrectNyushukkinMissingWaku(t *testing.T) {
	n := &stubNyushukkin{}
	_, err := CorrectNyushukkin(testApp(n, &stubWaku{}), "n1", nyushukkin.Input{
		Kind: "支出", Amount: "1", Date: "2026-09-06", WakuID: "w1",
	})
	...
	if len(n.items) != 0 {
		t.Fatalf("should not correct %+v", n.items)
	}
}
